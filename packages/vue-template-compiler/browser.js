...
function defineReactive (
  ...
  /**
   * 订阅者模式
   * 1. dep 为什么要在这里实例化, 就是为了实现, 对象每一层的 每一个 key 都有自己的一个订阅实例
   * 2. 比如 a.b 对应 dep1, a.c 对应 dep2, 这里虽然都是 const dep = new Dep(), 但每次来到这个方法, dep 都是独立的, 会一直保留在内存. 这样在每次调用set方法都能找到这个a.b对应的dep
   * 3. dep 这里会一直保存, 是因为闭包的关系, Object.defineProperty 这个全局的函数, 引用了上层的作用域, 这个作用域包含了 dep, 除非 Object = null, 或者退出浏览器, dep才会消失
   * 4. 实例化之后, dep 就有了被订阅, 和发布消息的功能, dep 不写在这里也是可以的, 多定义一个全局函数, 每次 observe 的时候增加一个 dep
   */
  var dep = new Dep();
  ...
  // 演示图 http://7xlolm.com1.z0.glb.clouddn.com/all.png
  Object.defineProperty(obj, key, {
    enumerable: true,
    configurable: true,
    get: function reactiveGetter () {
      ...
      if (Dep.target) {
        // 每次 new Watch('a.b', cb), 都会先执行get方法, 进而来到这里, 触发 dep.depend(), 这个dep就是 a.b 对应的 订阅,
        // dep.addSub() => this.subs.push(sub)
        dep.depend();
        if (childOb) {
          childOb.dep.depend();
          if (Array.isArray(value)) {
...
function mergeDataOrFn (
  ...
  if (!vm) {
    ...
    /**
     * 这里vm且data都不为空，所以会走到else if，返回的是mergedInstanceDataFn方法
     * 关于mergedInstanceDataFn方法，我们都知道，子组件中定义data时，必须是一个函数. 这里简单的判断了是函数就执行，不是就返回自身的值
     * 然后通过mergeData去合并，其实就是递归把defaultData合并到instanceData，并观察。
     *
     */
...
// ASSET_TYPES 就是 components、directives、filters，这三个的合并策略都一样，这里我们都返回了parentVal的一个子对象。
...
function addHandler (
  ...
  var newHandler = { value: value }; // newHandler = {'value': 'show'}
...
// 因为指令都是添加到模板的标签上的，所以第一步都是需要经过模板编译的洗礼
// 下面的是 标签处理的流程
 ...
 *
 * @param el
 * 解析其他执行和属性 比如 v-bind v-on 以及普通属性等
 * 会遍历所有的属性，整体上分成两部分：
 * 1. 指令     => v-bind， v-on，其他指令
 * 2. 普通属性  =>
 *
 */
  ...
  for (i = 0, l = list.length; i < l; i++) {
    ...
    if (dirRE.test(name)) {
      ...
      if (bindRE.test(name)) { // v-bind
        ...
        // '@click'.replace(/^@|^v-on:/, '') => 'click'
        name = name.replace(onRE, '');
        addHandler(el, name, value, modifiers, false, warn$1);
      } else { // normal directives 通用指令
...
var baseOptions = {
  expectHTML: true,
  modules: modules, // 包括 class 和 style，对模板中类和样式的解析
  directives: directives, // model（v-model）、html（v-html）、text(v-text)三个指令。
...
// 普通指令在生成render函数字符串时，会添加到data数据上，且在data的第一位。
function genData$2 (el, state) {
  ...
  // event handlers
  // DOM事件的render字符串生成DOM事件的render字符串生成
  // 可以看到，无论el.events还是el.nativeEvents都会通过genHandlers方法生成字符串，最终的数据是添加在data上的
  if (el.events) {
...
function genDirectives (el, state) {
  ...
  for (i = 0, l = dirs.length; i < l; i++) {
    dir = dirs[i];
    needRuntime = true;
    // 处理平台相关的一些指令；web端包含 v-html, v-model, v-text, v-bind, v-cloak
    var gen = state.directives[dir.name];
    // 自定义指令 gen => false, 所以needRuntime永远是true
    if (gen) {
    ...
    // 然后拼接好数据，添加到res上
    // 对于上面这五个指令（web端指令），只有v-model返回的是true，其它的指令不会添加到directive数组中在patch过程中处理
    if (needRuntime) {
      hasRuntime = true;
...
function createCompilerCreator (baseCompile) {
  return function createCompiler (baseOptions) {
    ...
    function compile (
      template,
      options // options 在内部主要是用户自己定义的 delimiters (标识符)
    ) {
      console.log('compiler -->>>');
      // finalOptions 继承自我们上面提到的baseOptions，
      var finalOptions = Object.create(baseOptions); // 自我们上面提到的baseOptions （src/platforms/web/compiler/options.js）
      var errors = [];
      var tips = [];
      // 并添加了一个搜集错误的warn方法
